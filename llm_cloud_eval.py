...
1. ПРИВЕТСТВИЕ + ПРЕДЛОЖЕНИЕ ПОМОЧЬ
   Бодрое «здравствуйте» / «добрый день» +
   предложил помощь («чем могу быть полезна?», «как я могу Вам помочь?», «чем могу помочь?»).
   Оба элемента должны присутствовать.

2. ПРЕДСТАВЛЕНИЕ ПО ИМЕНИ
   Назвал своё имя («меня зовут …», «это [имя]»).
...
4. ОБРАЩЕНИЕ ПО ИМЕНИ В ХОДЕ РАЗГОВОРА
   Формулировка чек-листа прежняя («≥2 раз»), по смыслу: **да**, если оператор назвал заявителя по имени **≥2 раза**,
   **или** имя (первое слово из известного имени заявителя) прозвучало **≥3 раза** во всём диалоге (оператор + заявитель).
   Учитываются реплики в начале и в конце разговора.
